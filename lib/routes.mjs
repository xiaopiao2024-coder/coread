...
export async function handleRequest(req, res, opts = {}) {
  ...
  if (req.method === 'POST' && req.url.match(/^\/v1\/books\/\d+\/comment$/)) {
    try {
      ...
      const result = db.prepare("INSERT INTO book_comments (book_id, paragraph_idx, sel_start_idx, sel_end_idx, sel_end_para_idx, selected_text, from_who, content, reply_to, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, datetime('now', '+8 hours'))").run(id, paragraph_idx, startIdx, endIdx, sel_end_para_idx ?? null, selected_text || null, author, content, reply_to ?? null);
      db.close();
      json(res, 200, { ok: true, id: Number(result.lastInsertRowid) });
      if (opts.onComment) opts.onComment({ book_id: id, paragraph_idx, from_who: author, content, selected_text: selected_text || null, comment_id: Number(result.lastInsertRowid) });
    } catch (e) { json(res, 500, { error: e.message }); }
    return true;
  }
  ...
  if (req.method === 'PATCH' && req.url.match(/^\/v1\/books\/\d+\/progress$/)) {
    try {
      ...
      const db = getDb();
      db.prepare("INSERT INTO book_progress (book_id, page, updated_at) VALUES (?, ?, datetime('now', '+8 hours')) ON CONFLICT(book_id) DO UPDATE SET page = ?, updated_at = datetime('now', '+8 hours')").run(id, page, page);
      db.close();
      json(res, 200, { ok: true });
    } catch (e) { json(res, 500, { error: e.message }); }
    return true;
